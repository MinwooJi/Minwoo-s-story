    ...
    <div class="form-box">
      <div class="real">
        <div class="mfeed-image">
          <!-- <div class="mimage-wrap"> -->
            ...
            <img
              :src="'https://jobara.s3.ap-northeast-2.amazonaws.com/snspost//' + detailPost.POST_NUMBER"
              alt="메인피드사진"
            />
          <!-- </div> -->
        </div>
      ...
      <div class="info-wrap">
        <div class="user-info">
          ...
          <div class="user-name">
            <p @click="otherUserDetail(detailPost.USER_ID)">{{detailPost.USER_ID}}</p>
          </div>
        </div>
      </div>
        ...
        <div class="feed-content-header">
          ...
          <div class="feed-content-like">
            <div class="like-count">
              <p>좋아요: {{ detailPost.POST_LIKE }}개</p>
            </div>
          </div>
        </div>

        <div class="m-text">{{detailPost.POST_SNS_CONTENT}}</div>
        <div>
          <ul>
            <li
...
export default {
  ...
  methods: {
    checklike() {
      let data = {
        postNumber: this.detailPost.POST_NUMBER,
        targetuserId: this.$session.get("loginUserId"),
        userId: this.detailPost.USER_ID
      };
      ...
      LikeApi.deleteStarPost(
        ...
        res => {
          ...
          if (res.data == "success") {
            ...
            this.fullBEmo = "none";
            console.log("=====>");
            this.$emit("updateBookMark", this.detailPost);
          }
        },
        error => {
          console.log(error);
    ...
    submitComment() {
      ...
      let data = {
        ...
        postNumber: this.detailPost.POST_NUMBER,
        userId: this.$session.get("loginUserId"),
        targetuserId: this.detailPost.USER_ID
        // commentNumber: 0,
      };
      // console.log(data)
      let CtempData = {
