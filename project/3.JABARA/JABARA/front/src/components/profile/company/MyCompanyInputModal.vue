    ...
    <div class="form-box">
      ...
      <div class="middle-wrap">
        ...
        <div class="c-input input-job">
          ...
          <input v-model="companyUrl" type="text" id="companyUrl" />
        </div>
        <div class="c-input input-area">
          <label for="companyImage" class="image-title">공고 이미지</label>
          <div id="fileupload">
            <div v-if="!changeimage" style="text-align: center " class="img-input-wrap">
              <input ref="file" type="file" @change="onFileChange" class="file-input" />
            </div>
            <div v-else class="success-image">
              <img :src="changeimage" width="80%" height="auto"/>
              <button class="remove-button" @click="removeImage">사진 삭제하기</button>
            </div>
          </div>
        </div>
      </div>
