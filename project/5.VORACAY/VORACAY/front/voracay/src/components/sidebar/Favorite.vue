      ...
      <table class="table table-hover">
        ...
        <tbody v-if="userList.length">
          <tr v-for="(item, index) in userList" :key="index + 'user_number'">
            ...
            <td class="call"><img src="../../assets/image/card/table/통화.png" alt="voda 로고" width="20" height="20" /></td>
          </tr>
        </tbody>
        <tbody v-else>
          <label> 표시할 정보가 없습니다</label>
        </tbody>
      </table>
    </div>
    <div class="btn-cover">
    ...
    setUser(category, search) {
      ...
      http
        ...
        .then((res) => {
          ...
          if(isNaN(total_count)) total_count = 1;
          this.pageSize = parseInt((total_count) / 10);
          
          if(this.userList == 'None') this.userList = [];
          if(total_count % 10 != 0) this.pageSize += 1;
          
          console.log(res);

          for (let i = 0; i < this.userList.length; i++) {
            this.userList[i].isFavorite = true;
          }
